...
def test_the_critic_dream_moves_only_critic_seams_and_the_actor_only_actor_seams() -> None:
    ...
    ac = cd.DreamActorCritic(
        ...
        pop,
        cd.learning_rule(dt=1.0),
        cd.DreamConfig(q_scale=1.0, batch=8, warmup=8, eta_critic=1.0, eta_actor=1.0),
        seed=1,
    )
    drive = ac.engine.clamp_levels(
        np.pad(np.random.default_rng(1).random((8, 3)), ((0, 0), (0, w.n - 3)))
    )
    before = ac.engine.edge_scale.copy()
    ac._dream(
        ac._clamp_action(drive, np.zeros((8, 1))),
        np.full(8, 0.7),
        ac.critic_mask,
        1.0,
        owners=ac.no_actor,
